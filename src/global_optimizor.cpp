...
void GlobalOptimizor::Options::LoadFromYaml(const std::string& config_fname) {
    try {
...
void GlobalOptimizor::AddScan(ScanFrame::Ptr scan) {
    scans_[scan->scan_id] = scan;
}

void GlobalOptimizor::ScanFilter() {
    keyscans_.clear();
    if (scans_.empty()) {
...
void GlobalOptimizor::BundleAdjustment() {
    // convert the points
...
void GlobalOptimizor::PoseGraphOptimize() {
    if (keyscans_.empty()) {
    ...
    for (auto it = keyscans_.begin(); it != keyscans_.end();) {
        ...
        problem.AddResidualBlock(cost, nullptr, scan_i->world_from_body.QuatData(), scan_i->world_from_body.PosData(),
                                 scan_j->world_from_body.QuatData(), scan_j->world_from_body.PosData());
        it = it_next;
    }
    ...
    if (options_.lc_enable) {
        for (const auto& [pair, pair_data] : loops_buf) {
            ...
            problem.AddResidualBlock(cost, nullptr, scan_a->world_from_body.QuatData(), scan_a->world_from_body.PosData(),
                                     scan_b->world_from_body.QuatData(), scan_b->world_from_body.PosData());
        }
    }
...
std::unordered_map<ScanPair, PairData> GlobalOptimizor::DetectLoopClosure() {
...
void GlobalOptimizor::SaveLoopToPcd(const std::string& save_fname) {
...
Trajectory GlobalOptimizor::ExportStampedPoses() {
...
PointCloud::Ptr GlobalOptimizor::GetSubMap(const scan_t& scan_id) {
    CHECK(keyscans_.count(scan_id));
