...
StampedPose::StampedPose() : time(0), pose(Eigen::Isometry3d::Identity()) {}

StampedPose::StampedPose(double t, const Eigen::Isometry3d &p) : time(t), pose(p) {}

Trajectory TrajectoryGenerator::linear(const Eigen::Vector3d &start_pos, const Eigen::Vector3d &velocity,
...
Trajectory TrajectoryGenerator::circular(const Eigen::Vector2d &center, double radius, double linear_vel, double height,
                                         double duration, double dt) {
    Trajectory traj;
...
Trajectory TrajectoryGenerator::load_from_tumtxt(const std::string &filename) {
    Trajectory traj;
...
void TrajectoryGenerator::save_to_tumtxt(const Trajectory &traj, const std::string &filename) {
...
void TrajectoryGenerator::save_to_pcd(const Trajectory &traj, const std::string &filename, double axis_length,
                                      int points_per_axis) {
...
TrajectoryInterpolator::TrajectoryInterpolator(const Trajectory &traj) : traj_(traj) {
    if (traj_.empty()) throw std::invalid_argument("trajectory is empty");
}

double TrajectoryInterpolator::t_start() const {
    return traj_.front().time;
}

double TrajectoryInterpolator::t_end() const {
    return traj_.back().time;
}

Eigen::Isometry3d TrajectoryInterpolator::query(double t) const {
...
const Trajectory &TrajectoryInterpolator::trajectory() const {
    return traj_;
}
